...
class DataPreprocessingMid():
    ...
    def main(self):
        ...
        re.to_csv('./data/mid/' + self.dealing + '.csv', index=0)
        return re
...
class DataPreprocessingReady():
    ...
    def get_history(self, data, uid_set):
        ...
        for uid in tqdm.tqdm(uid_set):
            ...
            seen_item_len = len(pos)
            indices = list(range(seen_item_len))
            if seen_item_len < 13 or seen_item_len > 100:
                continue
            random.shuffle(indices)
            supp_pos_seq = []
            for i in indices[:-10]:
                supp_pos_seq.append(pos[i])
            supp_pos[uid] = supp_pos_seq

            quer_pos_seq = []
            for i in indices[-10:]:
                quer_pos_seq.append(pos[i])
            quer_pos[uid] = quer_pos_seq
        return pos_seq_dict, supp_pos, quer_pos
    ...
    def split(self, src, tgt):
        ...
        pos_seq_dict, supp_pos, quer_pos = self.get_history(src, co_users)
        train_meta = tgt[tgt['uid'].isin(co_users - test_users)]

        train_support = tgt[tgt['uid'].isin(co_users - test_users)]
        train_query = tgt[tgt['uid'].isin(co_users - test_users)]
        train_meta['pos_seq'] = train_meta['uid'].map(pos_seq_dict)
        test['pos_seq'] = test['uid'].map(pos_seq_dict)

        train_support['supp_seq'] = train_support['uid'].map(supp_pos)
        train_query['quer_seq'] = train_query['uid'].map(quer_pos)
        train_supp = train_support.dropna(axis=0)
        train_quer = train_query.dropna(axis=0)
        return train_src, train_tgt, train_meta, test, train_supp, train_quer

    def save(self, train_src, train_tgt, train_meta, test, train_supp, train_quer):
        output_root = './data/ready/_' + str(int(self.ratio[0] * 10)) + '_' + str(int(self.ratio[1] * 10)) + \
        ...
        train_supp.to_csv(output_root + '/train_supp.csv', sep=',', header=None, index=False)
        train_quer.to_csv(output_root + '/train_quer.csv', sep=',', header=None, index=False)

    def main(self):
        src = self.read_mid(self.src)
        tgt = self.read_mid(self.tgt)
        src, tgt = self.mapper(src, tgt)
        train_src, train_tgt, train_meta, test, train_supp, train_quer = self.split(src, tgt)
        self.save(train_src, train_tgt, train_meta, test, train_supp, train_quer)
